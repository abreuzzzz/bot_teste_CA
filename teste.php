...
$GOOGLE_SHEETS_FILE_ID_Bluefields = getenv('GOOGLE_SHEETS_FILE_ID_Bluefields');
// Inicia a sessão
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<div class="topbar">
        ...
        <div class="menu-buttons">
            <a href="Bluefields.php" class="menu-button">
...
<div class="chart">
  ...
  <div class="chart">
    ...
    const GOOGLE_SHEETS_FILE_ID_Bluefields = '<?php echo $GOOGLE_SHEETS_FILE_ID_Bluefields; ?>';
    
        const CSV_URL = `https://docs.google.com/spreadsheets/d/${GOOGLE_SHEETS_FILE_ID_Bluefields}/export?format=csv&gid=1038646143`;
...
async function loadSimuladorData() {
    ...
    try {
        const response = await fetch(`/api-proxy_Bluefields.php?year=${year}`);
...
function criarGraficoRunwayIndicadores() {
    const fluxoMensal = {};
    const hoje = new Date();
    const mesAtual = `${hoje.getFullYear()}-${String(hoje.getMonth() + 1).padStart(2, '0')}`;

    rawData.forEach(row => {
        const date = getDateForRow(row, 'realizadoProjetado');
        const mk = getYearMonthFromDate(date);
        if (!mk || mk > mesAtual) return; // só até o mês atual, todos os status
        if (!fluxoMensal[mk]) fluxoMensal[mk] = { receita: 0, despesa: 0 };
        if (row.tipo === 'Receita') fluxoMensal[mk].receita += row.total || 0;
        else if (row.tipo === 'Despesa') fluxoMensal[mk].despesa += row.total || 0;
    });

    const mesesOrdenados = Object.keys(fluxoMensal).sort();
    if (mesesOrdenados.length === 0) return;

    // Saldo acumulado (posição de caixa)
    let saldoAcumulado = 0;
    const saldos = [];
    mesesOrdenados.forEach(mk => {
        saldoAcumulado += (fluxoMensal[mk].receita - fluxoMensal[mk].despesa);
        saldos.push(parseFloat(saldoAcumulado.toFixed(2)));
    });
    ...
    mesesOrdenados.forEach((mk, i) => {
        ...
        // Média das despesas dos últimos 3 meses
        const janela = mesesOrdenados.slice(Math.max(0, i - 2), i + 1);
        const mediaDespesas = janela.reduce((acc, m) => acc + fluxoMensal[m].despesa, 0) / janela.length;

        let runway = null;
        if (mediaDespesas > 0) {
            runway = parseFloat((saldos[i] / mediaDespesas).toFixed(1));
        }

        valoresRunway.push(runway);
    });
    ...
    ctx.chartInstance = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                tension: 0.4,
                spanGaps: true,
                pointBackgroundColor: valoresRunway.map(v =>
                    v === null ? '#94a3b8' :
                    v <= 0     ? '#f87171' :
                    v < 3      ? '#f87171' :
                    v < 6      ? '#fbbf24' : '#4ade80'
        ...
        options: {
                ...
                tooltip: {
                    callbacks: {
                        label: ctx => {
                            ...
                            if (v <= 0) return `Runway: ${v}m (caixa negativo)`;
                            return `Runway: ${v} meses`;
                        },
                        afterLabel: ctx => {
                            const i = ctx.dataIndex;
                            const mk = mesesOrdenados[i];
                            const { receita, despesa } = fluxoMensal[mk];
                            const janela = mesesOrdenados.slice(Math.max(0, i - 2), i + 1);
                            const mediaDespesas = janela.reduce((acc, m) => acc + fluxoMensal[m].despesa, 0) / janela.length;
                            return [
                                `Posição de Caixa: ${formatCurrency(saldos[i])}`,
                                `Média Despesas (3m): ${formatCurrency(mediaDespesas)}`,
                                `Receita do mês: ${formatCurrency(receita)}`,
                                `Despesa do mês: ${formatCurrency(despesa)}`
                            ];
                        }
                    }
                }
            ...
            scales: {
                y: {
                    ...
                    title: { display: true, text: 'Meses de Runway', color: '#94a3b8' }
                },
                x: {
                    grid: { display: false },
