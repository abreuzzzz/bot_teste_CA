...
from fluxo_lancamento import iniciar_selecao, callback_selecao, receber_texto_livre, estados
...
def _ok(update: Update) -> bool:
    return update.effective_user and update.effective_chat.id == TELEGRAM_ALLOWED_CHAT_ID


# ─── Comandos ─────────────────────────────────────────────────────────────────

async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not _ok(update): return
...
async def cmd_pendentes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not itens:
        ...
        return
    linhas = "\n".join(
        f"{'📥' if i['tipo']=='RECEBER' else '📤'} {i.get('descricao','?')[:35]} — R$ {i.get('valor',0):.2f} ({i.get('data_vencimento','')})"
        for i in itens[:20]
    )
    await update.message.reply_text(f"📅 *Pendentes (30 dias):*\n\n{linhas}", parse_mode="Markdown")
...
async def cmd_atrasados(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not itens:
        ...
        return
    linhas = "\n".join(
        f"⚠️ {i.get('descricao','?')[:35]} — R$ {i.get('valor',0):.2f} ({i.get('data_vencimento','')})"
        for i in itens[:20]
    )
    await update.message.reply_text(f"🚨 *Atrasados:*\n\n{linhas}", parse_mode="Markdown")
...
async def callback_forcar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not dados:
        ...
        return
    await query.edit_message_text("⏳ Lançando...")
    # Bypass anti-duplicata: cria diretamente sem checar
    from contaazul_client import _post, _rateio, _sanitizar
    ...
    parcelas_body = [{
        ...
        "nota": "Lançamento automático (forçado)",
    ...
    from contaazul_client import BASE
    r = _post(f"{BASE}/{endpoint}", body)
    await query.edit_message_text(f"✅ Lançado! ID: `{r.get('id','?')}`", parse_mode="Markdown")
...
async def handle_mensagem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Verifica se está no fluxo de seleção
    ...
    if acao in ("RECEBER", "PAGAR"):
        await iniciar_selecao(update, context, {
            ...
            "vencimento":  dados.get("vencimento", str(__import__('datetime').date.today())),
        ...
        await update.message.reply_text(
            f"🔎 Buscando lançamentos para dar baixa... (use /atrasados e informe o ID manualmente por enquanto)"
        )

    elif acao == "CONSULTA":
        resumo = resumo_mes()
        ctx    = json.dumps({k: v for k, v in resumo.items() if k not in ("contas_receber","contas_pagar")}, ensure_ascii=False)
        resp   = responder_consulta(texto, ctx)
        await update.message.reply_text(resp)

    else:
        msg = dados.get("mensagem", "")
        await update.message.reply_text(msg or "Não entendi. Tente descrever o lançamento ou use /manual.")


# ─── Handler de imagem/PDF ────────────────────────────────────────────────────
...
async def handle_documento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if acao in ("RECEBER", "PAGAR"):
        ...
        await update.message.reply_text(dados.get("mensagem", "Não consegui extrair os dados do documento."))
...
async def handle_foto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if acao in ("RECEBER", "PAGAR"):
        ...
        await update.message.reply_text(dados.get("mensagem", "Não consegui extrair os dados da imagem."))
...
async def _handle_manual(update: Update, context: ContextTypes.DEFAULT_TYPE, manual: dict):
    ...
    if etapa == "titulo":
        ...
        await update.message.reply_text("📅 Qual a *data de vencimento*? (ex: 15/05/2026)", parse_mode="Markdown")
        ...
        await update.message.reply_text("🔢 Quantas *parcelas*? (1 para à vista)", parse_mode="Markdown")
    elif etapa == "parcelas":
        try:
...
def main():
    ...
    import fluxo_lancamento
    fluxo_lancamento.InlineKeyboardMarkup = InlineKeyboardMarkup
    fluxo_lancamento.InlineKeyboardButton = InlineKeyboardButton
